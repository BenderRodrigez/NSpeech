    ...
    internal class Pitch:ISpeechFeature
    {
        ...
        private double[] TrackPitch()
        {
            ...
            foreach (var curentMark in _speechMarks)
            {
                ...
                for (int samples = curentMark.Item1; samples + size < _signal.Samples.Length && samples < curentMark.Item2; samples += jump)
                {
                    ...
                    for (int i = 1; i < acfsSample.Samples.Length - 1; i++)
                    {
                        if (acfsSample.Samples[i] > acfsSample.Samples[i - 1] && acfsSample.Samples[i] > acfsSample.Samples[i + 1])
                        {
                            acfsCandidates.Add(new Tuple<int, double>(i, acfsSample.Samples[i]));
                        }
                    }

                    for (int i = higher; i < acf.Samples.Length && i < lower; i++)
                    {
                        if (acf.Samples[i - 1] > acf.Samples[i - 2] && acf.Samples[i - 1] > acf.Samples[i])
                        {
                            candidates.Add(new Tuple<int, double>(i - 1, acf.Samples[i - 1]));//add each maximum of function from 60 to 600 Hz
                        }
                    }

                    var aproximatedPosition = acfsCandidates.Any() ? acfsCandidates[0].Item1 : -1;
                    var freqPosition = (_signal.SignalFormat.SampleRate / furieSize) * aproximatedPosition;//aproximated frequency value
        ...
        private void ExtractPitch(List<double> img, IReadOnlyList<List<Tuple<int, double>>> globalCandidates, int sampleRate, double furieSize, int jumpSize)
        {
            if (img.Count == 0)
            {
                return;
            ...
            var prevVal = 0.0;
            for (int i = BlurDiameter / 2; i < img.Count - BlurDiameter / 2; i++)//use median filter to cath the errors
            {
                var itemsToSort = new List<double>(BlurDiameter);
                for (int j = -BlurDiameter / 2; j <= BlurDiameter / 2; j++)
                {
            ...
            for (int i = img.Count - BlurDiameter / 2; i < img.Count; i++)//use median filter to cath the errors in the last points
            {
                var itemsToSort = new List<double>(BlurDiameter);
                for (int j = -BlurDiameter / 2; j <= BlurDiameter / 2; j++)
                {
