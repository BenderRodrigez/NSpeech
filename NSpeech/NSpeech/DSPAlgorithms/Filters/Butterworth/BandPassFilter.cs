...
namespace NSpeech.DSPAlgorithms.Filters.Butterworth
{
    ...
    internal sealed class BandPassFilter : ButterworthFilter
    {
        ...
        {
            var lowFreq = 2.0*Math.Sin(Math.PI*CutFrequencyLow/_sampleRate)/
                          Math.Cos(Math.PI*CutFrequencyLow/_sampleRate);
            var highFreq = 2.0*Math.Sin(Math.PI*CutFrequencyHigh/_sampleRate)/
                           Math.Cos(Math.PI*CutFrequencyHigh/_sampleRate);
